
#!/usr/bin/env Rscript

library(plyr)
library(ggplot2)
library(magrittr)
library(data.table)
library(patchwork)

## options
options(scipen = 999)
args <- commandArgs(trailingOnly = TRUE)
sbs52_mutsig_colours = c("#98D7EC", "#212121", "#FF003A", "#A6A6A6", "#F5ABCC")

if (length(args) == 0) {
  cat("No arguments provided.\n")
} else {
    # import
    germline_sbs52_df = fread(args[1])
    germline_normalised_sbs52_df = fread(args[2])
    species_name = args[3]
    species_name = paste(unlist(strsplit(species_name, "_")), collapse = " ")
    sample_name = args[4]
    sample_name = sprintf(" (%s)", sample_name)
    output = args[5]
    
    # setnames
    setnames(germline_normalised_sbs52_df, 4, "COUNT")
    
    # manipulate
    germline_sbs52_df$TRI <- sapply(strsplit(germline_sbs52_df$TRI, " "), function(x) x[2])
    germline_normalised_sbs52_df$TRI <- sapply(strsplit(germline_normalised_sbs52_df$TRI, " "), function(x) x[2])

    # ggplot2
    sbs52_plot = ggplot(germline_sbs52_df, aes(x=TRI, y=COUNT, fill=SUB)) +
        geom_bar(stat="identity")  +
        theme_bw(24) + # maybe
        facet_grid(. ~ SUB, scales="free") + 
        scale_fill_manual(values = sbs52_mutsig_colours) +
        labs(
             x = "\nTrinucleotide\n", 
             y = "\nCounts\n",
             title = bquote(italic(.(species_name)) * .(sample_name) * " germline mutational spectrum")
        ) +
        theme(
          axis.text.x = element_text(family = "mono", angle = 90, hjust = 1),
          axis.line = element_line(color = "black", linewidth = 1, linetype = "solid"),
          legend.position = "none",
          legend.title = element_blank()
        )

    # ggplot2
    normalised_sbs52_plot = ggplot(germline_normalised_sbs52_df, aes(x=TRI, y=COUNT, fill=SUB)) +
        geom_bar(stat="identity")  +
        theme_bw(24) + # maybe
        facet_grid(. ~ SUB, scales="free") + 
        scale_fill_manual(values = sbs52_mutsig_colours) +
        labs(
             x = "\nTrinucleotide\n", 
             y = "\nNormalised Counts\n",
             title = bquote(italic(.(species_name)) * .(sample_name) * " normalised germline mutational spectrum")
        ) +
        theme(
          axis.text.x = element_text(family = "mono", angle = 90, hjust = 1),
          axis.line = element_line(color = "black", linewidth = 1, linetype = "solid"),
          legend.position = "none",
          legend.title = element_blank()
        )

    # combine plots
    plot = sbs52_plot / normalised_sbs52_plot 

    # ggsave
    ggsave(output, plot, width = 24, height = 28)
}
