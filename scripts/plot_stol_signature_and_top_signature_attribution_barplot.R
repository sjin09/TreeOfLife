...
if (length(args) == 0) {
  cat("No arguments provided.\n")
    ...
    # import
    somatic_signature_df = fread(args[1])
    somatic_attribution_df = fread(args[2])
    sig_name = args[3]
    output = args[4]
   
    # manipulate data
    somatic_attribution_df = somatic_attribution_df[order(ATTRIBUTION, decreasing=TRUE)]
    somatic_attribution_df[, x_label_expr := mapply(function(id, sp) {
      paste0("paste('", id, "', ' (', italic('", sp, "'), ')')")
    }, SAMPLE, SPECIES)]
    somatic_attribution_df[, x_label_expr := factor(
      x_label_expr, 
      levels = x_label_expr[order(ATTRIBUTION, decreasing = TRUE)]
    )]
     
    # ggplot2
    sig_plot = ggplot(somatic_signature_df, aes(x=TRI, y=PROBABILITY, fill=SUB)) +
    ...
    # ggplot2
    sig_attribution_plot = ggplot(somatic_attribution_df[1:100,], aes(x=x_label_expr, y=ATTRIBUTION)) +
        geom_bar(stat="identity")  +
        theme_bw(24) +
        scale_fill_manual(values = sbs96_mutsig_colours) +
        labs(
